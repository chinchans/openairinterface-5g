...
extern void nr_HO_F1_inter_du_ltm_trigger_telnet(gNB_RRC_INST *rrc, uint32_t rrc_ue_id);
/**
...
int rrc_gNB_trigger_f1_inter_du_ltm_ho(char *buf, int debug, telnet_printfunc_t prnt)
{
  if (!RC.nrrrc)
    ERROR_MSG_RET("no RRC present, cannot list counts\n");
  rrc_gNB_ue_context_t *ue = NULL;
  ...
  gNB_RRC_UE_t *UE = &ue->ue_context;
  nr_HO_F1_inter_du_ltm_trigger_telnet(RC.nrrrc[0], UE->rrc_ue_id);
  prnt("RRC F1 Inter-gNB DU LTM handover (UE Context Setup) triggered for UE %u\n", UE->rrc_ue_id);
  return 0;
}
...
static telnetshell_cmddef_t cicmds[] = {
    ...
    {"trigger_f1_inter_du_ltm_ho", "[rrc_ue_id(int,opt)]", rrc_gNB_trigger_f1_inter_du_ltm_ho},
